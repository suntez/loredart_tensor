...
Tensor _reduceTensor(
  Tensor x,
  num Function(List, List<int>, List<int>, List<int>, DType) reduceRule, {
  List<int>? axis,
  bool keepDims = false,
  DType? dType,
}) {
  if (x is NumericTensor) {
    final List<int> cumProd = List<int>.generate(
      x.rank,
      (i) =>
          i == x.rank - 1
              ? (x.shape[i] == 1 ? 0 : 1)
              : (x.shape[i] == 1 ? 0 : x.shape.list.sublist(i + 1).reduce((e1, e2) => e1 * e2)),
    );
    dType ??= x.dType;
    if (axis == null || axis.isEmpty) {
      ...
      for (int i = 0; i < axis.length; i += 1) {
        if (axis[i] >= x.rank || axis[i] <= -x.rank) {
          throw RangeError.value(
            axis[i],
            'axis',
            'Axis should include elements from interval [-x.rank, +x.rank), but got axis[$i] = ${axis[i]}',
          );
        }
        axis[i] %= x.rank;
      }
      ...
      if (axis.length > x.rank || axis.length < axis.toSet().length) {
        throw ArgumentError(
          'Axis.length mast be less or equal x.rank and must contain unique elements, but got $axis',
          'axis',
        );
      }
      ...
      if (shape.every((element) => element == 1)) {
        ...
        for (int i = 0; i < buffer.length; i += 1) {
          ...
          buffer[i] = reduceRule(
            x.buffer,
            List.generate(x.rank, (i) => shape[i] == 1 ? x.shape[i] : currentIndices[i]),
            cumProd,
            axis,
            dType,
          );
        ...
        if (!keepDims) {
          shape = [
            for (int i = 0; i < shape.length; i += 1)
              if (!(shape[i] == 1 && axis.contains(i))) shape[i],
          ];
        }
...
/// ```
Tensor reduceMax(Tensor x, {List<int>? axis, bool keepDims = false}) =>
    _reduceTensor(x, reduceMaxSlice, axis: axis, keepDims: keepDims, dType: x.dType);
...
/// ```
Tensor reduceMin(Tensor x, {List<int>? axis, bool keepDims = false}) =>
    _reduceTensor(x, reduceMinSlice, axis: axis, keepDims: keepDims, dType: x.dType);
...
/// ```
Tensor reduceSum(Tensor x, {List<int>? axis, bool keepDims = false}) =>
    _reduceTensor(x, reduceSumSlice, axis: axis, keepDims: keepDims, dType: x.dType);
...
/// ```
Tensor reduceProd(Tensor x, {List<int>? axis, bool keepDims = false}) =>
    _reduceTensor(x, reduceProdSlice, axis: axis, keepDims: keepDims, dType: x.dType);
...
/// ```
Tensor reduceMean(Tensor x, {List<int>? axis, bool keepDims = false, DType? dType}) =>
    _reduceTensor(x, reduceMeanSlice, axis: axis, keepDims: keepDims, dType: dType ?? x.dType);
...
/// ```
Tensor reduceVariance(Tensor x, {List<int>? axis, bool keepDims = false, DType? dType}) =>
    _reduceTensor(x, reduceVarianceSlice, axis: axis, keepDims: keepDims, dType: dType ?? x.dType);
...
/// ```
Tensor reduceStd(Tensor x, {List<int>? axis, bool keepDims = false, DType? dType}) =>
    _reduceTensor(x, reduceStdSlice, axis: axis, keepDims: keepDims, dType: dType ?? x.dType);
...
/// ```
Tensor reduceLocalArgMax(Tensor x, {List<int>? axis, bool keepDims = false}) =>
    _reduceTensor(x, reduceLocalArgMaxSlice, axis: axis, keepDims: keepDims, dType: DType.int32);
