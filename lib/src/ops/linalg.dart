...
Tensor matmul(Tensor a, Tensor b, {bool transposeA = false, bool transposeB = false}) {
  if (a.rank != b.rank) {
    throw ArgumentError(
      'Tensors a and b must be of the same rank, but received a.rank: ${a.rank} and b.rank: ${b.rank}',
    );
  }
  ...
  for (int i = 0; i < a.rank - 2; i += 1) {
    if (a.shape[i] != b.shape[i]) {
      throw ArgumentError(
        'Tensors a and b must have same batch shape, but received a.shape: ${a.shape} and b.shape: ${b.shape}',
      );
    }
  }
  ...
  if ((a is NumericTensor) && (b is NumericTensor)) {
    ...
    List<int> shape = [...a.shape.list.sublist(0, a.rank - 2), aM, bL];
      ...
      for (int i = 0; i < aM; i += 1) {
        for (int j = 0; j < bL; j += 1) {
          ...
          for (int k = 0; k < aN; k += 1) {
            dot +=
                a.buffer[batch * aM * aN + (!transposeA ? (i * aN + k) : (k * aM + i))] *
    ...
    throw ArgumentError(
      'Tensors a and b must be NumericTensors of the same DType, but received ${a.dType} and ${b.dType}',
    );
  }
}
...
Tensor matrixTranspose(Tensor x) {
  if (x is NumericTensor && x.rank >= 2) {
    ...
    throw ArgumentError(
      'Expected NumericTensors with rank >= 2, but received ${x.runtimeType} of the rank: ${x.rank}',
      'x',
    );
  }
}
