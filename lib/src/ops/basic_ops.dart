...
Tensor opWithCompShape(NumericTensor x, NumericTensor other, ArithmeticOperation arithmeticOperation) {
  ...
  final List<int> cumProdT = List<int>.generate(
    shape.length,
    (i) => i == shape.length - 1 ? 1 : (x.shape[i] == 1 ? 0 : x.shape.list.sublist(i + 1).reduce((e1, e2) => e1 * e2)),
  );
  final List<int> cumProdOther = List<int>.generate(
    shape.length,
    (i) =>
        i == shape.length - 1
            ? 1
            : (other.shape[i] == 1 ? 0 : other.shape.list.sublist(i + 1).reduce((e1, e2) => e1 * e2)),
  );
...
Tensor opWithLastDims(NumericTensor x, NumericTensor other, ArithmeticOperation arithmeticOperation) {
  if (other.rank > x.rank) {
    throw ArgumentError(
      'Incorrect arguments order: expect to have x with higher rank than other, but received x.rank: ${x.rank} and other.rank: ${other.rank}',
    );
  }
  DType dType = dTypeDecision(x.dType, other.dType);
  List buffer = emptyBuffer(dType, x.shape.size);
